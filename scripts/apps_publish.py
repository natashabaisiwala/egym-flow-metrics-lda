...
No engine/methodology changes. No changes to data.json/data-core.json/
data-machine.json/data-wellpass.json. Apps has NO DORA page (build_pdf_realm.py's
NO_DORA_REALMS already includes "apps" — nothing to do here for that).
...
CORRECTIONS = (
    "Standard methodology includes bugs in throughput and defines tech% via "
    "customfield_10463 (not 11502). Some old manual reports for Workout and the "
    "now-retired BMA Enterprise excluded bugs and/or used a different tech "
    "definition — those old numbers are Corrections, the engine is correct. "
    "Evergreen/umbrella epics are excluded from FL2 delivered/cycle-time counts via "
    "the shared FL2_EXCLUDE (no Apps-specific entries exist yet)."
)
...
def publish(jira_conn, gh_conn, today, anchor_override=None, cadence_override=None,
    ...
    print(f"  computing {REALM} {month} (4 teams: bma_core_growth, bma_engagement, "
          f"trainer, workout) ...")
    tv = cj.compute_team_values(jira_conn, anchor, cj.APPS_TEAMS)
...
def deliver(slack_conn, today, pdf_path, anchor_override=None, cadence_override=None):
    ...
    print(f"  DELIVERED Apps {month_full} -> Slack {CHANNEL}")

...
(cgr/acq/aex/dis/pex/int/mot/act), cadence day 1 of the month following the
reported month (rolled to next business day, no buffer -- handled generically
by compute_jira.py's REALM_CADENCE/report_anchor(), no extra code needed here).
Since Aug 2026, FL1 (Tasks) is computed over the reported calendar month
(REALM_CALENDAR_MONTH_FL1 in compute_jira.py); FL2 (Epics) is unaffected and
still uses the rolling 120-day window. Same validation/anomaly gates as Core.
...
USAGE (run in sequence on the Wellpass cadence day, day 1 of the month
FOLLOWING the reported month, rolled to next business day on weekends). Since
the run date and the reported month are now different calendar months, always
pass --cadence=YYYY-MM explicitly for the month being reported (get both the
run date and --cadence from wellpass_cadence_check.py's RUN_TODAY output) —
omitting --cadence falls back to the run date's own month, which is wrong
under the new cadence:
  ...
  uv run --with numpy,tzdata python wellpass_publish.py <jira> <gh> <YYYY-MM-DD> --publish \
      --cadence=<YYYY-MM> [--force-anomalies] [--slack=<slack_conn>]
  ...
  uv run --with numpy,tzdata,reportlab python wellpass_publish.py <jira> <gh> <YYYY-MM-DD> \
      --report --cadence=<YYYY-MM> --notes=notes_wellpass.json --out=/agent/home/Wellpass_Realm_flow_metrics_YYYY_MM.pdf
  ...
  uv run --with numpy,tzdata python wellpass_publish.py <jira> <gh> <YYYY-MM-DD> \
      --deliver --cadence=<YYYY-MM> --slack=<slack_conn> --pdf=/agent/home/Wellpass_Realm_flow_metrics_YYYY_MM.pdf
...
# Direct chat link for this Wellpass realm agent -- used in anomaly-escalation
# messages so "this agent" is a clickable hyperlink for the tagged LDAs.
AGENT_CHAT_URL = "https://app.dataleap.ai/agents/a_nru4usmbp4rti49g8zho"
...
# Human-readable labels used ONLY for display in Slack anomaly/escalation messages.
# Purely cosmetic post-processing of report_checks.anomalies() output -- no engine
# code touched, no change to the underlying computed values or thresholds.
TEAM_NAMES = {
    "cgr": "Company Growth", "acq": "Offer & Acquisition", "aex": "Access Experience",
    "dis": "Discovery", "pex": "Partner Experience", "int": "Partner Integrations",
    "mot": "Motivation", "act": "Activation",
}
METRIC_LABELS = {
    ("fl1", "ct"): "FL1 Cycle time", ("fl1", "tp"): "FL1 Throughput",
    ("fl1", "tech"): "FL1 Tech tasks %", ("fl1", "bC"): "FL1 Bugs created",
    ("fl1", "bR"): "FL1 Bugs resolved", ("fl1", "WIP(total)"): "FL1 WIP (total)",
    ("fl2", "ct"): "FL2 Cycle time", ("fl2", "del"): "FL2 Epics delivered",
    ("fl2", "wip"): "FL2 WIP", ("fl2", "tech"): "FL2 Tech roadmap %",
}
import re as _re
_ANOM_RE = _re.compile(r'^(?P<tk>\w+)\.(?P<fl>fl1|fl2)\.(?P<metric>[\w\(\)]+): '
                       r'(?P<pv>\S+) -> (?P<cv>\S+) \(large change\)$')
_ANOM_NOPREV_RE = _re.compile(r'^(?P<tk>\w+): no previous-month data to compare \(new team\?\)$')

def _humanize_anomaly(a):
    """Best-effort: render a raw report_checks anomaly string with real team names
    and human-readable metric labels. Falls back to the raw string unchanged if
    the format doesn't match (never crashes, never drops data)."""
    m = _ANOM_RE.match(a)
    if m:
        team = TEAM_NAMES.get(m.group("tk"), m.group("tk"))
        label = METRIC_LABELS.get((m.group("fl"), m.group("metric")),
                                   f"{m.group('fl').upper()} {m.group('metric')}")
        return f"{team} \u2014 {label}: {m.group('pv')} \u2192 {m.group('cv')} (large change)"
    m = _ANOM_NOPREV_RE.match(a)
    if m:
        team = TEAM_NAMES.get(m.group("tk"), m.group("tk"))
        return f"{team}: no previous-month data to compare (new team?)"
    return a
...
# One-time trigger for the FL1 methodology + cadence redesign (calendar-month
# population replacing the rolling 120-day window, Average WIP replacing the WIP
# Aging Risk / WIP Snapshot charts and FL1 WIP KPI card, cadence moved to day 1
# of the month following the reported month). Aug 2026 is the first calendar
# month actually reported under the new rules (delivered Sept 1, 2026). Using an
# explicit month label here -- not an index, unlike SEED_MONTH_COUNT -- because
# the backfilled Jul 26 month is recomputed under the new rules too but is never
# itself delivered as a live report, so an index-based check would fire on the
# wrong month.
FL1_METHODOLOGY_CUTOVER_MONTH = "Aug 26"

# One-time Corrections narrative — covers BOTH boundaries this note can fire for
# (manual-seed-to-engine, and the FL1 methodology/cadence redesign) in a single
# widened text, since in practice only the FL1_METHODOLOGY_CUTOVER_MONTH trigger
# will realistically fire (Aug 26 is well past the seed boundary). Attached only
# to the one month it applies to (see build_report() below) — never re-flagged
# afterwards.
CORRECTIONS = (
    "Two boundaries meet in this report. (1) The migrated May 2025 - Jun 2026 history "
    "(13 months) was manually seeded from Nave-verified reports via the older "
    "egym-flow-metrics-live repo, not recomputed from Jira (see Data provenance in "
    "REALM_WELLPASS.md) -- every month from Jul 2026 onward is computed directly by the "
    "automated engine. (2) Starting with this report, Wellpass FL1 (Tasks) metrics -- "
    "Cycle Time, Throughput, Bugs, and Tech % -- are computed over the reported calendar "
    "month instead of a rolling 120-day window, the FL1 WIP Aging Risk and WIP Snapshot "
    "charts are replaced by a single Average WIP metric, and the report cadence moves to "
    "day 1 of the month following the reported month. FL1 KPI headline cards continue to "
    "show a rolling-120-day smoothed figure so a single month can't swing the headline "
    "number by itself; the FL1 charts below show the real calendar-month data, including "
    "its natural month-to-month swings. FL2 (Epics) is entirely unaffected by this change "
    "and still uses the rolling 120-day window throughout. Because this report's FL1 "
    "figures come from a different population than earlier months, they may not be "
    "directly comparable to those earlier months -- that is expected, not a fresh data "
    "anomaly."
)
...
def _escalate_anomalies(slack_conn, month_full, anom):
    ...
    lines = "\n".join(f"\u2022 {_humanize_anomaly(a)}" for a in anom)
    msg = (f"\U0001F6A8 *Wellpass Realm \u2014 anomaly review needed \u00b7 {month_full}*\n"
           ...
           f"{lines}\n\n"
           f"{tags} \u2014 please review: summarize your evergreen/methodology assessment and "
           f"chat with <{AGENT_CHAT_URL}|this agent> directly with a decision (force-publish "
           f"as-is, or exclude specific epic keys with sign-off). No data was written; this "
           f"run is ending now.")
...
def build_report(gh_conn, today, notes_path, out_path, anchor_override=None,
    ...
    rv = copy.deepcopy(rd)
    # One-time Corrections note: fires for EITHER of two boundaries -- the
    # manual-seed-to-engine boundary (index-based, SEED_MONTH_COUNT) or the FL1
    # calendar-month/cadence methodology cutover (label-based,
    # FL1_METHODOLOGY_CUTOVER_MONTH). In practice only the latter will realistically
    # fire (Aug 26 is well past the seed boundary), but both checks are kept so
    # neither boundary is ever silently missed if backfill timing shifts. Both
    # point at the same widened CORRECTIONS text above, so there's nothing to
    # merge/dedupe even if both conditions were ever true for the same month.
    if months.index(month) == SEED_MONTH_COUNT or month == FL1_METHODOLOGY_CUTOVER_MONTH:
...
def deliver(slack_conn, today, pdf_path, anchor_override=None, cadence_override=None):
    ...
    msg = (f"\U0001F4CA *Wellpass Realm \u2014 Flow Metrics \u00b7 {month_full}*\n"
           ...
           f"_Automatically generated \u00b7 FL1 calendar month + FL2 rolling 120-day window "
           f"(through {report_date})_")
    call_tool("slack_post_message", {
        "connectionId": slack_conn, "channelId": CHANNEL, "message": msg,

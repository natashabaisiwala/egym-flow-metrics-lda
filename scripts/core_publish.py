...
WIP_KEYS_PATH = "data-core-wip-keys.json"
...
def _write_delivery_manifest(gh_conn, anchor, month, month_full, report_date, tv=None):
    gen.GH_CONN = gh_conn
    ...
    def snap_keys(fl, tid):
        """WIP-aging Red/Yellow/Green issue-key buckets for the just-computed month
        ONLY, sourced from the in-memory tv (compute_jira.compute_team_values output).
        Never available for fl*_prev -- the public data-core.json schema persists
        only aggregate wR/wY/wG counts per month, never per-key membership, so
        historical months' keys cannot be reconstructed after the fact. See
        compute_jira.py's _ryg_keys()/_ryg_weekly_keys() (manifest-persistence fix,
        Alexa-approved). This manifest is a local delivery_core.json artifact only --
        it is never merged back into data-core.json's public per-month arrays."""
        if not tv or tid not in tv or fl not in tv[tid]:
            return None
        f = tv[tid][fl]
        if not all(k in f for k in ("wR_keys", "wY_keys", "wG_keys")):
            return None
        return {"wR": f["wR_keys"], "wY": f["wY_keys"], "wG": f["wG_keys"]}
    ...
    for tid, t in rd["teams"].items():
        teams_out[tid] = {"name": t.get("name", tid),
                          ...
                          "fl1_now_keys": snap_keys("fl1", tid),
                          "fl2_now_keys": snap_keys("fl2", tid)}
    manifest = {
...
def _persist_wip_keys(gh_conn, month, tv):
    """Durably persist this month's WIP Aging Risk issue-key buckets (wR/wY/wG, both
    fl1 and fl2, per team) to a DEDICATED repo file (WIP_KEYS_PATH), so future
    walkthroughs/action-item requests can cite exact Jira keys directly instead of
    inferring them from ticket age. Separate file from data-core.json -- never
    touches that file's public per-month schema (hard rule: no data.json schema
    changes). Idempotent: skips (no push) if `month` already has non-null data.
    Sourced from the in-memory tv (compute_jira.compute_team_values output) for the
    just-computed month only -- months published before this feature existed have
    no entry here and cannot be reconstructed after the fact."""
    try:
        r = call_tool('github_get_file_contents', {
            'connectionId': gh_conn, 'owner': OWNER, 'repo': REPO, 'path': WIP_KEYS_PATH})
        doc = json.loads(base64.b64decode(r['content'].replace('\n', '')).decode())
        sha = r['sha']
    except Exception:
        doc = {"realm": REALM, "months": {}}
        sha = None

    if doc.get("months", {}).get(month):
        print(f"  wip-keys: {month} already present in {WIP_KEYS_PATH} — idempotent skip (no push).")
        return {"status": "skipped"}

    month_out = {}
    for tid in cj.CORE_TEAMS:
        if tid not in tv:
            continue
        entry = {}
        for fl in ("fl1", "fl2"):
            f = tv[tid].get(fl, {})
            if all(k in f for k in ("wR_keys", "wY_keys", "wG_keys")):
                entry[fl] = {"wR": f["wR_keys"], "wY": f["wY_keys"], "wG": f["wG_keys"]}
        if entry:
            month_out[tid] = entry
    if not month_out:
        print("  wip-keys: no key data available on tv (engine key-tracking missing?) — skipping push.")
        return {"status": "no_data"}

    doc.setdefault("months", {})[month] = month_out
    content = json.dumps(doc, ensure_ascii=False, separators=(',', ':'), sort_keys=True)
    kwargs = {'connectionId': gh_conn, 'owner': OWNER, 'repo': REPO, 'path': WIP_KEYS_PATH,
              'message': f"WIP-keys (auto) {REALM}: add {month}", 'content': content}
    if sha:
        kwargs['sha'] = sha
    call_tool('github_create_or_update_file', kwargs)

    # Byte-verify (hard rule): fetch back and diff before declaring success.
    back = call_tool('github_get_file_contents', {
        'connectionId': gh_conn, 'owner': OWNER, 'repo': REPO, 'path': WIP_KEYS_PATH})
    back_content = base64.b64decode(back['content'].replace('\n', '')).decode()
    if back_content != content:
        print("  ! wip-keys push FAILED byte-verification — fetched content does not match what was sent.")
        return {"status": "verify_failed"}
    print(f"  wip-keys: {month} pushed + byte-verified to {WIP_KEYS_PATH}")
...
def publish(jira_conn, gh_conn, today, anchor_override=None, cadence_override=None,
    ...
    manifest = _write_delivery_manifest(gh_conn, anchor, month, month_full, report_date, tv=tv)
    print(f"  DELIVERY PENDING {REALM} {month} — wrote {os.path.basename(manifest)}; "
          f"author notes_core.json then run --report and --deliver")

    wk = _persist_wip_keys(gh_conn, month, tv)
    print(f"  wip-keys result: {wk['status']}")

    return {"status": "updated", "month": month, "wip_keys": wk["status"]}

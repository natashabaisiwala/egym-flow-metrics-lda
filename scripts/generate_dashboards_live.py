...
# ─── GitHub helpers ────────────────────────────────────────────────────────────
def load_data(path='data-machine.json'):
    """path — which realm data file to load, e.g. 'data-core.json'. Defaults to
    'data-machine.json' for backward compatibility with existing Machine callers."""
...
def run(gh_conn, realm_ids=None, data_path='data-machine.json'):
    """
    gh_conn    — GitHub connection ID
    realm_ids  — list of realm IDs to update, e.g. ['core']. None = all realms with data
                 IN data_path's file (each realm data file is expected to hold only its
                 own realm(s), so leave realm_ids=None or pass the single matching id).
    data_path  — which realm data file to load, e.g. 'data-core.json'. Defaults to
                 'data-machine.json' for backward compatibility with existing Machine callers.
    ...
    # Trim every realm to the last DISPLAY_MONTHS months for rendering (full history
    # is preserved in data.json on GitHub; we only slice the in-memory copy here).
    for _r in data['realms'].values():
        _trim_realm(_r)

    print("Fetching file SHAs...")
    shas = load_shas()

    realms_to_build = realm_ids or [rid for rid,r in data['realms'].items() if r['months']]
    print(f"Building realms: {realms_to_build}")

    for rid in realms_to_build:
        r = data['realms'][rid]
        months = r['months']
        if not months:
            print(f"  ⏭  {r['name']}: no data yet, skipping")
            continue

        print(f"\n📁 {r['name']} ({len(months)} months, {len(r['teams'])} teams)")

        # Team dashboards
        for tid, team in r['teams'].items():
            has_first_metric = bool(team['fl1'].get('wip')) if rid in WELLPASS_REALMS else bool(team['fl1'].get('ct'))
            if not has_first_metric:
                print(f"  ⏭  {team['name']}: no data")
                continue
            slug = slugify(team['name'])
            path = f"{rid}/{slug}.html"
            html = team_html(rid, r['name'], tid, team, months, fl2_data=_team_fl2(r, team))
            push(path, html, f"Update {team['name']} ({months[-1]})", shas.get(path))
            print(f"  ✅ {path} ({len(html)//1024}KB)")

        # Realm-level aggregate dashboard
        rd_path = f"{rid}/realm-dashboard.html"
        push(rd_path, realm_dashboard_html(rid, r),
             f"Update {r['name']} realm dashboard ({months[-1]})", shas.get(rd_path))
        print(f"  ✅ {rd_path}")

        # Realm index
        idx_path = f"{rid}/index.html"
        push(idx_path, realm_index_html(rid, r),
...
if __name__ == '__main__':
    import sys
    realm_arg = sys.argv[2].split(',') if len(sys.argv) > 2 else None
    run(sys.argv[1] if len(sys.argv) > 1 else input("GitHub connectionId: "), realm_arg)
